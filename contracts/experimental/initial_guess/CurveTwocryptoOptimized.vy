...
def add_liquidity(
    ...
    D: uint256 = MATH.newton_D(A_gamma[0], A_gamma[1], xp, 0)#                  TODO: can use use old_D here?

    token_supply: uint256 = self.totalSupply
    if old_D > 0:
        d_token = token_supply * D / old_D - token_supply
    else:
        # Make initial virtual price equal to 1:
        d_token = self.get_xcp(D, price_scale)
    ...
    if old_D > 0:
        ...
        # Initialise xcp oracle here as virtual_price * totalSupply / 10**18:
        self.cached_xcp_oracle = d_token
...
def remove_liquidity_one_coin(
    ...
    packed_price_scale: uint256 = self.tweak_price(A_gamma, xp, D, 0)           # NOTE: Here we don't need an initial
    #        Safe to use D from _calc_withdraw_one_coin here ---^                       guess since new_D has already
    #                                                                                   been calculated, and will be
    #                                                                                   used as an initial guess should
    #                                                                                   the algorithm decide to rebalance
    #                                                                                   position.
    # ------------------------- Transfers ------------------------------------
...
def _exchange(
    ...
    if t > block.timestamp:
        ...
        xp[i] = x0                                                         # |
        self.D = MATH.newton_D(A_gamma[0], A_gamma[1], xp, 0)              # |  TODO: can we use self.D here?
        xp[i] = x1  # <-------------------------------------- ... and restore.
...
def tweak_price(
    ...
    @param initial_D Initial guess of D value for `newton_D`.
    """
    ...
    if virtual_price * 2 - 10**18 > xcp_profit + 2 * rebalancing_params[0]:
        ...
        if norm > adjustment_step:  # <---------- We only adjust prices if the
            ...
            #                                                                   NOTE: We are also using D_unadjusted
            #                                                                         here as an initial guess!
            D: uint256 = MATH.newton_D(
                A_gamma[0],
                A_gamma[1],
                xp,
                D_unadjusted,  # <--------------------------------------------  NOTE: Previously we did not use any
            )  #                                                                      initial guesses.
