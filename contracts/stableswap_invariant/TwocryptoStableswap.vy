...
def add_liquidity(
    ...
    if self.future_A_gamma_time > block.timestamp:  # <--- A_gamma is ramping.
        ...
        old_D = staticcall MATH.get_D(xp_old, A_gamma[0])

    else:

        old_D = self.D

    D: uint256 = staticcall MATH.get_D(xp, A_gamma[0])
...
def _exchange(
    ...
    if t > block.timestamp:
        ...
        xp[i] = x0                                                         # |
        self.D = staticcall MATH.get_D(xp, A_gamma[0])              # |
        xp[i] = x1  # <-------------------------------------- ... and restore.
    ...
    D: uint256 = self.D
    y_out: uint256 = staticcall MATH.get_y(i, j, xp[i], initial_xp, A_gamma[0], D)
    dy = xp[j] - y_out
    xp[j] -= dy
    dy -= 1
    ...
    # STABLESWAP removed initial guess
    price_scale = self.tweak_price(A_gamma, xp, 0, 0)
...
def tweak_price(
    ...
    if new_D == 0:  #  <--------------------------- _exchange sets new_D to 0.
        D_unadjusted = staticcall MATH.get_D(_xp, A_gamma[0])

    # ----------------------- Calculate last_prices --------------------------

    self.last_prices = unsafe_div(
        (staticcall MATH.get_p(_xp, A_gamma[0], D_unadjusted))[0] * price_scale,
        10**18
    )
...
# TODO reimplement one coin withdrawal
