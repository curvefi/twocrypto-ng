...
@settings(**SETTINGS)
def test_last_price_remove_liq(pool_with_deposit, user, token_frac, i):
    prices = INITIAL_PRICES
    token_amount = token_frac * pool_with_deposit.totalSupply() // 10**18

    with boa.env.prank(user):
        pool_with_deposit.remove_liquidity_one_coin(token_amount, i, 0)

    oracle_price = pool_with_deposit.last_prices()
...
@settings(**SETTINGS)
def test_ma(pool_with_deposit, coins, user, amount, i, t):
    ...
    with boa.env.prank(user):
        pool_with_deposit.exchange(i, 1 - i, amount, 0)

    prices2 = pool_with_deposit.last_prices()
    boa.env.time_travel(t)

    with boa.env.prank(user):
        pool_with_deposit.remove_liquidity_one_coin(10**15, 0, 0)

    prices3 = pool_with_deposit.price_oracle()
...
@settings(**SETTINGS)
def test_price_scale_range(pool_with_deposit, coins, user, amount, i, t):
    prices1 = INITIAL_PRICES
    amount = amount * 10**18 // prices1[i]
    boa.deal(coins[i], user, amount)
    ...
    prices3 = pool_with_deposit.price_scale()
