...
def test_exchange_all(
    ...
    calculated = views_contract.get_dy(i, j, amount, pool_with_deposit)
...
def test_exchange_received_success(
    ...
    with boa.env.prank(user):
        ...
        out = pool_with_deposit.exchange_received(i, j, amount, int(0.999 * calculated), user)
...
def test_exchange_received_revert_on_no_transfer(
    ...
    with boa.env.prank(user), boa.reverts("!coins"):
        pool_with_deposit.exchange_received(i, j, amount, int(0.999 * calculated), user)
