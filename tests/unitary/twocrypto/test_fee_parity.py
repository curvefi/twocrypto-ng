...
def test_compare_fee_parity(pool, pool_with_fee_contract):
    legacy_pool = GodModePool(pool)
    hooked_pool = GodModePool(pool_with_fee_contract)

    assert legacy_pool.EXT_FEE() == "0x0000000000000000000000000000000000000000"
    assert hooked_pool.EXT_FEE() != "0x0000000000000000000000000000000000000000"
