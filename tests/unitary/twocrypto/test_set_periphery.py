...
def test_set_views_only(pool, factory_admin):
    ...
    assert pool.VIEW() == mock_view, "View contract not set correctly"
    # assert pool.MATH() == boa.eval("empty(address)"), "Math contract should remain empty"
...
def test_set_math_only(pool, factory_admin):
    ...
    current_view = pool.VIEW()

    mock_math = boa.env.generate_address()
    pool.set_periphery(boa.eval("empty(address)"), mock_math, sender=factory_admin)

    assert pool.VIEW() == current_view, "View contract should not change"
    assert pool.MATH() == mock_math, "Math contract not set correctly"
...
def test_update_existing_contracts(pool, factory_admin):
    ...
    assert pool.VIEW() == new_view, "View contract not updated"
    assert pool.MATH() == initial_math, "Math contract should not change"

    # Update only math
    new_math = boa.env.generate_address()
    pool.set_periphery(boa.eval("empty(address)"), new_math, sender=factory_admin)

    assert pool.VIEW() == new_view, "View contract should not change"
    assert pool.MATH() == new_math, "Math contract not updated"
